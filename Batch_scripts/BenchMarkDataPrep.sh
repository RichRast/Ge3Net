#!/bin/bash
# srun --pty -p bigmem --mem=1100G -c 1 --time 24:00:00 ./Batch_scripts/BenchMarkDataPrep.sh 2>&1 | tee $OUT_PATH/humans/benchmark/benchMarkDataPrep_log.txt
source ini.sh
...
cat <(tail -n +2 $OUT_PATH/humans/labels/$dataset/train_sample_map.tsv) <(tail -n +2 $OUT_PATH/humans/labels/$dataset/valid_sample_map.tsv) | cut -f 1,3 >$OUT_PATH/humans/benchmark/$dataset/train_valid/sample_map.txt

#remove header from test sample map tsv
sed '1d' $OUT_PATH/humans/labels/$dataset/test_sample_map.tsv|cut -f 1,3 > $OUT_PATH/humans/benchmark/$dataset/test/sample_map.txt
...
# use bcftools to subset the vcf file with sample maps for train val and test for 1_geo
for split in $StringVal; do
