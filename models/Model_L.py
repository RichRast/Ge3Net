...
class model_L(object):
    def __init__(self, *args, params):
        ...
        self.att_network2 = args[4]
        self.FFNN_network2 = args[5]
    ...
    def train(self, optim, training_generator, writer=None, wandb=None):
        ...
        self.att_network2.train()
        self.FFNN_network2.train()
        # making sure no gradient flow through the last fc layer of first FFNN
        self.FFNN_network.fc3.requires_grad=False
        ...
        for i, train_gen in enumerate(training_generator):
            ...
            input_x2, _, weight1 = self.att_network(self.PE_network(input_x1))
            output_0, output_1 = self.FFNN_network(input_x2)
            input_x3, _, weight2 = self.att_network2(output_0)
            _, output_3 = self.FFNN_network2(input_x3)

            preds.append(output_3*cp_mask)
            ...
            loss_main = self.criterion(output_3*cp_mask, train_labels*cp_mask)
        ...
        train_result=results(accr = train_accr, pred=None, weights=[weight1, weight2])
    ...
    def valid(self, validation_generator, writer=None, wandb=None):
        ...
        self.att_network2.eval()
        self.FFNN_network2.eval()
        ...
        with torch.no_grad():
            ...
            for j, val_gen in enumerate(validation_generator):
                ...
                for _ in range(self.params.mc_samples):
                    ...
                    output_list.append(output_3)
                    
                outputs_cat = torch.cat(output_list, 0).contiguous().\
                view(self.params.mc_samples, -1, self.params.dataset['n_win'], output_3.shape[-1]).mean(0)
                
                outputs_var = torch.cat(output_list, 0).contiguous().\
                view(self.params.mc_samples, -1, self.params.dataset['n_win'], output_3.shape[-1]).var(0)
            ...
            eval_result = results(accr = accuracy, pred = [y_pred, cp_pred_out, y_pred_var], weights=[weight1, weight2])
