...
class model_P(object):
    def __init__(self, aux_network, main_network, main_network2):

        self.aux_network = aux_network
        self.main_network = main_network
        self.main_network2 = main_network2
        #self.criterion = torch.nn.MSELoss()
        self.accr_criterion1 = torch.nn.MSELoss(reduction='sum')
        self.criterion = torch.nn.L1Loss()
        self.criterion2 = torch.nn.BCELoss()
        self.accr_criterion2 = torch.nn.L1Loss(reduction='sum')
    ...
    def train(self, optimizer, training_generator, params, writer=None):
        ...
        for i, train_gen in enumerate(training_generator):
            ...
            cp_mask = cp_mask.to(params.device)
            # Forward pass
            # update the gradients to zero
            optimizer.zero_grad()
            
            out1, out7, _, out9 = self.aux_network(train_x)
            loss_aux = self.criterion(out9*cp_mask, train_labels*cp_mask)
            ...
            if params.rnn_net['tbptt']:
                for text_chunk, batch_label_chunk in zip(bptt_batch_chunks, batch_label):
                    ...
                    tmp_accr_sum = self.accr_criterion2(train_vector, batch_label_chunk)
                    accr.append(tmp_accr_sum)
            else:
                att_train_x, att_scores, weights = self.main_network2(train_lstm)
                _,train_vector, _ = self.main_network(att_train_x)
                loss_main = self.criterion(train_vector*cp_mask, train_labels*cp_mask)
                cp_pred = self.cp_network(train_vector)
                loss_cp = self.criterion2(cp_pred, cp_mask)
                
                tmp_accr_sum = self.accr_criterion2(train_vector, train_labels)
                accr.append(tmp_accr_sum)
            ...
            if params.rnn_net['tbptt']:
                ...
                loss = loss_main + loss_aux + loss_cp
                loss.backward()
        ...
        with torch.no_grad():
            ...
            for j, val_gen in enumerate(validation_generator):
                ...
                for _ in range(params.mc_samples):
                    ...
                    val_loss_regress_MLP = self.criterion(out9*cp_mask, val_labels*cp_mask)
                    ...
                    att_val_x, att_scores, weights = self.main_network2(val_lstm)
                    
                    _, val_outputs, _ = self.main_network(att_val_x)
                ...
                tmp_accr_sum = self.accr_criterion2(outputs_cat, val_labels)
                accr.append(tmp_accr_sum)
