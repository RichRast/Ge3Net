...
parser.add_argument('--data.params_dir', type=str, help='directory where json file for model hyperparameters are stored')
...
parser.add_argument('--data.genetic_map', type=str, default=osp.join( os.environ.get('IN_PATH'), 'reference_files/allchrs.b38.gmap'),
                    help='directory where genetic map is saved')
parser.add_argument('--data.sample_map', type=str, default='', metavar='sample_map_type',
                help="type of sample map such as expt1, a, b c or keep")
...
parser.add_argument('--data.expt_id', type=str, default=1, help='unique experiment identifier seed')
...
parser.add_argument('--data.ref_filter_criteria', type=str, help='criteria to filter the reference map')
parser.add_argument('--data.method', type=str, default='pca', help='method to create labels')
parser.add_argument('--data.n_comp_overall', type=int, help='overall components of pca')
parser.add_argument('--data.n_comp_subclass', type=int, help='subclass components of extended pca')
parser.add_argument('--data.n_comp', type=int, help='total number of components (relevant for extended pca)')

#form labels
parser.add_argument('--data.create_labels', type=distutils.util.strtobool, default='True', help='set True if you want to form labels')
parser.add_argument('--data.labels_dir', type=str, help='dir for labels')
...
parser.add_argument('--data.all_chm_snps', type=str, default=osp.join(os.environ.get('IN_PATH'), \
    'combined_chm/all_chm_combined_snps_variance_filter_0.3.npy'), metavar='all_chm',
...
# model arguments
parser.add_argument('--model.working_dir', type=str, help='directory where models related to experiment is saved')
