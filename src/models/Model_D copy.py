...
from src.models import AuxiliaryTask, LSTM, BasicBlock
...
class model_D(nn.Module):
    def __init__(self, params):
        ...
        self.aux=AuxiliaryTask.AuxNetwork(params)
        self.rnn=LSTM.BiRNN(params, rnn="lstm")
        self.changepoint=BasicBlock.logits_Block(params)

    def forward(self,x):
        out1, out2, out3, out4 = self.aux(x)
        ...
        vec_64, out_rnn, _ = self.rnn(out_nxt)
        cp_logits=self.changepoint(vec_64)
        return out4, out_rnn, cp_logits
    ...
    def _tbtt(self, x, target):
        ...
        for x_chunk, batch_label_chunk, cps_chunk in zip(bptt_batch_chunks, batch_label, batch_cps_chunks):
            ...
            vec_64, out_rnn_chunk, rnn_state = self.rnn(x_chunk, rnn_state)
            ...
            rnn_state = self.rnn._detach_rnn_state(rnn_state)
