...
class model_N(nn.Module):
    def __init__(self, params, criterion, cp_criterion):
        ...
        count_params=[]
        for m in [self.aux, self.lstm1, self.attention, self.ffnn, self.lstm2, self.cp]:
            if m is not None:
    ...
    def _setOptimizerParams(self):
        ...
        for i, m in enumerate([self.aux, self.lstm1, self.attention, self.ffnn, self.lstm2, self.cp]):
            if m is not None:
                params_dict={}
    ...
    def _batch_validate_1_step(self, val_x, **kwargs):
        ...
        mc_dropout = kwargs.get('mc_dropout')
        if mc_dropout is not None: activate_mc_dropout(*[self.aux, self.lstm1, self.attention, self.ffnn, self.lstm2, self.cp])
