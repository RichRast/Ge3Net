...
from src.models.Attention import TransformerModel, LinearLayers
...
class model_P(nn.Module):
    def __init__(self, params, criterion, cp_criterion):
        ...
        self.fc = LinearLayers(self.params, self.params.aux_net_hidden)
        ...
        count_params=[]
        for m in [self.aux, self.mht_encoder, self.fc, self.cp]:
            if m is not None:
    ...
    def _setOptimizerParams(self):
        ...
        for i, m in enumerate([self.aux, self.mht_encoder, self.fc, self.cp]):
            if m is not None:
                params_dict={}
    ...
    def forward(self, x, mask, **kwargs):        
        ...
        def _forwardNet(x):
            ...
            out_nxt = self.mht_encoder(out1)
            out_att = self.fc(out_nxt)
    ...
    def _batch_validate_1_step(self, val_x, **kwargs):
        ...
        mc_dropout = kwargs.get('mc_dropout')
        if mc_dropout is not None: activate_mc_dropout(*[self.aux, self.mht_encoder, self.fc, self.cp])
